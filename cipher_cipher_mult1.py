...
mxn matrix/nxn matrix multiplication
"""
...
def cipher_cipher_matmul(A_cipher, B_cipher, n, m, c):
    ...
    for j in range(m_tilde):
        c_init = A_cipher[(j)%(m//c)] * B_cipher[0]
        C_cipher[j] = c_init
        for ell in range(1, n):
            ctj_ell = rotate(A_cipher[(j + -1 * (ell // c))%(m//c)], -n * (ell % c) + ell)
...
if __name__ == "__main__":
    ...
    for m in m_array:
        ...
        for c in c_array:
            ...
            for j in range(m//c):
                for i in range(c):
                    ldv = lower_diagonal_vector(A, j * c + i) 
                    A_j_conc.append(ldv)
